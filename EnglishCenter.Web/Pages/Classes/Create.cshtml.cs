...
public class CreateModel : PageModel
{
    ...
    public async Task OnGetAsync()
    {
        ...
        var rooms = await _apiClient.GetAsync<PagedResult<RoomLookupDto>>("rooms?PageNumber=1&PageSize=1000");

        Courses = courses?.Items.Select(c => new SelectListItem(c.Name, c.Id.ToString())).ToList() ?? new List<SelectListItem>();
        var roomItems = rooms?.Items ?? new List<RoomLookupDto>();
        if (Input.CampusId.HasValue && Input.CampusId.Value > 0)
        {
            roomItems = roomItems.Where(r => r.CampusId == Input.CampusId.Value).ToList();
        }

        Rooms = roomItems
            .Select(r => new SelectListItem($"{r.Name} ({r.RoomCode})", r.Id.ToString()))
            .ToList();
